...
def check_depth(x, y):
    ...
    while start_time + 1.5 > time.time():
        ...
        check_array.append(img[y, x, 0])

    for depth in check_array:
        if depth > 4000:
            return True
...
while True : 
    ...
    # Get the depth value of a specific pixel
    if len(refPt) == 1:
        x, y = refPt[0]
        print(img[y, x, 0])  # Access the depth value of the pixel
        check_array.append(img[y, x, 0])

        # Print the depth value
        # print("Depth value at pixel (x={}, y={}): {}".format(x, y, depth_value))
